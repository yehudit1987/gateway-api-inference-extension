...
func TestHandleRequestHeaders(t *testing.T) {
	tests := []struct {
		...
		{
			name: "extracts headers and returns response on end of stream",
			headers: &extProcPb.HttpHeaders{
				Headers: &basepb.HeaderMap{
					Headers: []*basepb.HeaderValue{
						{Key: "content-type", RawValue: []byte("application/json")},
...
func TestHandleRequestBody_MultiPluginHeaderMutations(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			want := buildStreamingResponse(tc.wantSetHeaders, tc.wantRemoved, bodyBytes)
...
// buildStreamingResponse constructs the expected ProcessingResponse for
// HandleRequestBody call with the given header mutations.
func buildStreamingResponse(setHeaders map[string]string, removeHeaders []string, bodyBytes []byte) []*extProcPb.ProcessingResponse {
